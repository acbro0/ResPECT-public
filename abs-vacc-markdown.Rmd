...
vc <- ggplot(vac_count, aes(x=predictors, y = exp(Estimate))) +                     
  ...
  labs(y = "Adjusted relative rate of HCW absenteeism", x = NULL) +                 
...
vz <- ggplot(vac_zero, aes(x=predictors, y = exp(Estimate))) +                         
  ...
  coord_flip() +               
  geom_hline(aes(yintercept=1), lty=2) +           
  labs(y = "Adjusted odds ratios for HCW taking any sick time", x = NULL) +                 
  theme(axis.text = element_text(size=14),  axis.title=element_text(size=16,face="bold")) +
...
ggplot(vac, aes(x=predictors, y = exp(Estimate))) +                                                   theme_bw() + 
  geom_pointrange (aes (ymin = exp(Estimate-Std..Error), ymax = exp(Estimate+Std..Error))) +              
  coord_flip() +               
  geom_hline(aes(yintercept=1), lty=2) +           
  labs(y = "Adjusted odds ratios for HCW influenza vaccination", x = NULL) +                 
  theme(axis.text = element_text(size=14),  axis.title=element_text(size=16,face="bold")) +
